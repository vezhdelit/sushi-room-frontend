...
const AllItems = () => {
  ...
  const getItems = async () => {
    const category =
      categoryType && categoryType !== "Всі" ? `&category=${categoryType}` : "";
  ...
  const renderedItems = items.map(
    (obj) =>
      (!showOnlyFavourites || (data && data.favourites.includes(obj._id))) && (
  ...
  return (
    <div className={styles.container}>
      <h2 className={styles.title}>
        {showOnlyFavourites ? "Тільки улюблені" : "Всі позиції"}
      </h2>
      {status === "error" ? (
        <div>
